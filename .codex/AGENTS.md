...
3. **PWA Integration**: Maintain caching rules inside [sw.js](../apps/web/public/sw.js) for assets such as `favicon.svg`, `icon-192.png`, and `icon-512.png`.

## Code Quality Standards

- **Functional Cohesion**: Keep components small. Split visual elements into specialized files under `apps/web/src/components/`.
...
- **Audit Contract**: Ensure the integrity of the proof verification path in [App.jsx](../apps/web/src/App.jsx). The `tamperAgreement` logic must correctly simulate malicious database tampering and prompt warning mismatch flags on both mobile client passes and the Mentor telemetry ledger.
