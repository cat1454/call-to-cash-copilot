...
| Phase 10.5 CSV schedule fixture is Excel-editable, imports 25-30 May rows, and rolls past May dates to the next future year.                                                   | `prisma/schedule-fixture.test.ts`      | PASS   |
...
corepack pnpm exec tsx --test prisma/schedule-fixture.test.ts
...
`corepack pnpm --filter @call-to-cash/api typecheck` is currently blocked by pre-existing
untracked/debug files under `apps/api/src/test-*.ts` that import relay sources outside the API
`rootDir` and reference removed Agora config fields. The focused API test for this slice passed.
